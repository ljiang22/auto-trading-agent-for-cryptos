 ...
 * Twitter's search syntax supports date filtering with since: and until: operators.
 ...
 * @param searchMode The search mode (Latest recommended for date-based searches)
 * @param auth The TwitterAuth object
 * @returns An async generator yielding Tweet objects
 */
export function searchTweetsByDate(
  query: string,
  startDate: string,
  endDate: string,
  maxTweets: number,
  ...
  const dateFilteredQuery = `${query} since:${startDate} until:${endDate}`;
  
  // Use the existing searchTweets function with our date-filtered query
  return searchTweets(dateFilteredQuery, maxTweets, searchMode, auth);
}

/**
 * Fetches a single page of tweets within a specific date range.
 * 
 * @param query The base search query
 * @param startDate The start date in YYYY-MM-DD format (inclusive)
 * @param endDate The end date in YYYY-MM-DD format (exclusive)
 ...
 * @returns A promise that resolves to a QueryTweetsResponse
 */
export async function fetchTweetsByDate(
  query: string,
  startDate: string,
  endDate: string,
  maxTweets: number,
  ...
  // Use the existing fetchSearchTweets function with our date-filtered query
  return fetchSearchTweets(dateFilteredQuery, maxTweets, searchMode, auth, cursor);
}
