...
describe("Embedding Module", () => {
    ...
    describe("embed function", () => {
        ...
        test("should return identical embeddings for the same input via LRU cache", async () => {
            // Two consecutive calls with the same input should yield the
            // same array reference: the second call hits the in-process
            // sha256-keyed LRU and skips the provider entirely.
            const a = await embed(mockRuntime, "lru test input");
            const b = await embed(mockRuntime, "lru test input");
            expect(b).toBe(a);
        });
