...
import { sanitizeProcessEnv } from "./envSanitizer.ts";
...
export function loadEnvConfig(): Settings {
    ...
    // Optional override: .env.local sits next to .env and wins for any key it defines.
    // Intended for per-developer secrets; must be in .gitignore.
    if (envPath) {
        const localPath = path.join(path.dirname(envPath), ".env.local");
        if (fs.existsSync(localPath)) {
            const localResult = config({ path: localPath, override: true });
            sanitizeProcessEnv(process.env);
            if (!localResult.error) {
                elizaLogger.log(`Loaded .env.local from: ${localPath}`);
            }
        }
    }
...
/**
 * Gets an environment variable with production override support
 * @param {string} key - The base environment variable key
 * @param {string} [defaultValue] - Optional default value if key doesn't exist
 * @returns {string|undefined} The environment variable value or default value
 */
export function getProductionEnvVariable(
    key: string,
    defaultValue?: string
): string | undefined {
    const isProduction = process.env.NODE_ENV === 'production';
    const productionKey = `PRODUCTION_${key}`;
    
    if (isBrowser()) {
        return isProduction 
            ? environmentSettings[productionKey] || environmentSettings[key] || defaultValue
            : environmentSettings[key] || defaultValue;
    }
    
    return isProduction 
        ? process.env[productionKey] || process.env[key] || defaultValue
