...
2c. **Recurring / scheduled / conditional strategy.** A strategy that runs OVER TIME cannot be fully expressed as one-shot orders. Recognize it by any of: a DCA **cadence** ("$100 every week / every two weeks / monthly"), dip-buys keyed to a **moving reference** ("buy $50 if BTC drops 5% from its 7-day high", "max 2 dip buys per month"), or **take-profit / stop-loss / pause** rules ("sell 25% at +20% profit", "pause new buys if down 15%"). FIRST decide whether the user wants the strategy to **run automatically** or just to **start it manually now**, then pick ONE of the two sub-paths:

   **2c-auto — AUTO-EXECUTION intent (PREFERRED when present).** The user wants the strategy to run on its own / automatically / continuously. Recognize it by words like: "auto-trade / auto-execute / automatically / on its own / keep buying / run it / run this strategy / arm it / arm this / set it running / let it run / automate this / have it trade for me / always-on" (zh: "自动 / 自动交易 / 自动执行 / 自动定投 / 帮我自动 / 一直买 / 挂着跑"). For these, emit a **2-step plan** — do NOT emit \`create_order\` tranches (the engine places them):
   1. \`compile_strategy\` — \`parameters.description\` = the full strategy in NL. Compiles the cadence + dip triggers + take-profit/stop-loss into a STRUCTURED DSL and saves it.
   2. \`arm_strategy\` — arms the compiled strategy for **PAPER auto-execution** via the strategy engine. This is the ONE approval gate; once armed, the engine evaluates the strategy on its cadence and places paper orders automatically.
   In the \`summary\`, state plainly: once you approve the arm step, the strategy runs AUTOMATICALLY in **PAPER mode** (no real money) — it evaluates on its cadence and places the recurring DCA + conditional dip-buys + take-profit/stop-loss as paper orders on its own. Live auto-execution is NOT permitted; a live request is downgraded to paper. Do NOT add \`create_order\` steps and do NOT say the legs "won't fire" — once armed they DO fire in paper.

   **2c-manual — NO auto-execution intent.** The user describes a recurring strategy but only wants to START it now / place the first buy (no "automatically / arm / run it" words). Emit a MULTI-STEP plan — NEVER a lone \`get_balance\` and NEVER a single order:
   ...
   3. \`compile_strategy\` — \`parameters.description\` = the full strategy in NL.
   ...
   In the \`summary\`, state plainly: (a) what executes NOW — ONLY the first tranche; and (b) that the recurring DCA + conditional dips + exits are compiled + backtested for review and will NOT fire on their own UNLESS the user arms them — invite the user to reply **"arm it"** to have the strategy engine run the rest automatically in paper. This is distinct from the one-time staged-buy case in 2b ("$300 now + $300 at -5%"), which has a fixed, finite set of legs mapped to \`create_order\`s directly.
...
3. **Use action names verbatim** from the "Available Actions" list. Common actions:
   ...
   - \`compile_strategy\` (compile an NL trading strategy into a STRUCTURED DSL for RECURRING/SCHEDULED/CONDITIONAL strategies — used for backtesting, as a saved record, and as the input to \`arm_strategy\`), \`run_backtest\` (evaluate a strategy against historical data) — read-only analysis; no approval. Pass the full strategy text in \`parameters.description\`.
   - \`arm_strategy\` (arm a compiled strategy for PAPER auto-execution via the strategy engine — the engine then evaluates it on its cadence and places paper orders automatically; paper-only, live is downgraded; requires ONE approval), \`pause_strategy\` / \`resume_strategy\` / \`stop_strategy\` (lifecycle control of a running strategy), \`list_strategies\` (read-only: show the user's strategies + status). Use \`arm_strategy\` whenever the user wants a strategy to run automatically / on its own (see rule 2c-auto).
...
Example (RECURRING strategy, NO auto-execution intent — rule 2c-manual → COMPILE + first tranche, invite arm):
...
\`\`\`json
{
  "summary": "Place the first $100 DCA tranche now (market buy) and compile + backtest the full Hybrid DCA + Risk-Control ruleset. ⚠️ ONLY the first $100 executes now. The recurring bi-weekly DCAs, the -5%-from-7d-high dip-buys, the 20% take-profit and the 15% stop-loss/pause are compiled + backtested for your review and will NOT fire on their own unless you arm the strategy; reply \\"arm it\\" to have the strategy engine run the rest automatically in PAPER mode, or place each future tranche manually.",
  "steps": [
