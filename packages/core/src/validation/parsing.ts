import type { ActionResponse } from "../core/types.ts";
const jsonBlockPattern = /```json\n([\s\S]*?)\n```/;

export const messageCompletionFooter = `\nResponse format should be formatted in a valid JSON block like this:
\`\`\`json
{ "user": "{{agentName}}", "text": "<string>", "action": "<string or array of strings>" }
\`\`\`

The "action" field should be one of the options in [Available Actions] or an array of actions if multiple actions are needed (e.g., ["getPrice", "getNews"]) and the "text" field should be the response you want to send.
`;
