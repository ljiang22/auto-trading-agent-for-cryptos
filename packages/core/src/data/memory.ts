...
export class MemoryManager implements IMemoryManager {
    ...
    async addEmbeddingToMemory(memory: Memory): Promise<Memory> {
        ...
        try {
            ...
            elizaLogger.error("Failed to generate embedding", {
                message: err.message,
                name: err.name,
                stack: err.stack,
                textLen: memoryText?.length,
            });
     ...
     * @param opts.roomId Optional room ID to filter memories by room. If not provided, searches globally.
     * @param opts.unique Whether to retrieve unique memories only.
