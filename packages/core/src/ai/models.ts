...
export const models: Models = {
    ...
    // ANTHROPIC and CLAUDE_VERTEX provider entries are kept for type-system
    // compatibility, but the model names point at Gemini equivalents because
    // every Anthropic API key was failing in prod (April 2026). New code should
    // target ModelProviderName.GOOGLE with Vertex
    // (GOOGLE_APPLICATION_CREDENTIALS_JSON); these blocks exist so any
    // residual ANTHROPIC selection still resolves to a working model name when
    // the provider switch in api.ts forwards to Google.
    [ModelProviderName.ANTHROPIC]: {
        endpoint: settings.ANTHROPIC_API_URL || "https://api.anthropic.com/v1",
        model: {
            [ModelClass.SMALL]: {
                name:
                    settings.SMALL_ANTHROPIC_MODEL || "gemini-2.0-flash-lite",
                stop: [],
