...
For Linux/AMD64 machines, you can now test the agent locally by updating the `TEE_MODE` environment variable to `DOCKER` and setting the environment variables in the compose manifest shown below. Once you have done that, you can start the agent by running:
...
Next, take the compose manifest below and edit it based on your agent configuration.
...
Click on the `Compose Manifest Mode` tab and paste the compose manifest content into the `Compose Manifest` field.
