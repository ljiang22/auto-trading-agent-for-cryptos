...
# Directory for dev logs (override with DEV_LOGS_DIR env var)
LOGS_DIR="${DEV_LOGS_DIR:-./logs/dev}"
mkdir -p "$LOGS_DIR"
RUN_TIMESTAMP="$(date +"%Y%m%d_%H%M%S")"
LOG_FILE="$LOGS_DIR/dev_${RUN_TIMESTAMP}_$$.log"

# Mirror all script output (stdout/stderr) to this run's log file.
...
WORKING_FOLDERS=("plugin-cex") # Core is handled separately
