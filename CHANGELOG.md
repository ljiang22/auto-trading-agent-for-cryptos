...
- **`unknown_state` informed consent** — kind-specific title, reconciliation-pending warning, `last_seen_at` in the dialog, and action guidance (check `/orders`, wait for reconciliation, or cancel as the safe default). Override remains user-approvable; other risk gates still apply.
- **`in_flight` override** — intentional: user may retry with a new client order ID after explicit approval when an in-flight duplicate is detected.
- **Telemetry** — `order_dispatch_attempt` now emits after dedup resolution so `client_order_id` matches the final derived ID.

## Autotrading safety + features — Wave 1 + Wave 2 (2026-05-19)

> Branch: `autotrading-wave1-2-jiggly-codd`. Plan:
> `/home/leon26/.claude/plans/based-on-what-you-jiggly-codd.md`.
> Operator contract: `docs/AUTOTRADING_SAFETY.md`.

### Wave 1 — Safety (launch blockers)

- **F1 — paper / live response disclosure**
  - `CEXWorkflowState.resolvedExecutionMode` persisted on the langgraph state
  - `getCEXResultFormattingTemplate` rewritten to require the mode badge as the first line of every paper / shadow response
  - Mechanical post-check `renderModeBadge` + `hasModeBadge` in `cexWorkflowMessageHandler` deterministically prefixes the badge if the SLM forgets
  - plugin-cex `templates/output.ts` + `error.ts` fork user-visible copy by mode; `TradeActionConfig` and `createTradeAction` thread the resolved mode through the action-response envelope (`metadata.actionData.mode`)
- **F2 — user-feature memory poisoning**
  - `UserFeatureManager.processMessage` filters recent messages by `classifyPromptInjection` verdict before LLM derivation
  - `validateAspectResponse` enforces a safety-bypass blocklist on derived aspects
  - Aspects derived from any trading-keyword batch are tagged `consentRequired=true` and excluded from `formatUserTraitsForContext` until the user opts in via Settings → Inferred Traits
  - New `IUserFeatureManager` methods: `listUserAspectsWithMemoryIds`, `setAspectConsent`, `deleteAspect`, `deleteAllUserAspects`
  - New endpoints: `GET /user/inferred-traits`, `PUT /user/inferred-traits/:memoryId/consent`, `DELETE /user/inferred-traits/:memoryId`, `DELETE /user/inferred-traits`
  - New Settings tab: `client/src/components/cex/InferredTraitsTab.tsx`
  - `[UserFeature]` audit log on every consent flip / delete
- **F3 — paper-order persistence**
  - New `paper_orders` and `paper_fills` MongoDB collections + indexes (`{userId, order_id}` unique, `{userId, status}`, `ttl_at` TTL)
  - New `PaperOrderStore` abstraction (in-memory for tests, adapter-backed in production)
  - `PaperVenueExchangeService` writes through the store; cached singleton per `realVenue` in `shared.ts` so orders persist across action calls and container restarts (24 h default TTL)
  - Read actions (`get_orders`, `get_balance`, `get_fills`) now route to the paper venue when `mode=paper`
  - `cancelOrder` distinguishes paper-found / paper-not-found; output template forks
  - SQLite parity in `packages/adapter-sqlite/src/index.ts`
  - Env: `PAPER_ORDER_TTL_SECONDS` (default `86400`)
- **F4 — trading audit event schema completion**
  - `CanonicalIntent` gains `notional_usd_estimated` (set from `deriveEstimatedNotionalUsd` pre-risk-check)
  - `intentFields()` now emits the full 12-field invariant set including `stake: intent.mode` and `notional_usd`
  - `emitRiskCheck` now fires unconditionally inside `runRiskPrecheck` with `latency_ms`
  - Legacy `stake: read_only|write` wire field renamed to `tool_capability` on `emitPreprocess` / `emitStakeCheck` / `emitIntentClassified` so the CLAUDE.md `stake` slot is reserved for execution mode (live/paper/shadow)
- **F5 — reconciliation per-user credentials**
  - `FallbackPollerConfig.resolveCredentials: (userId, venue) => Promise<… | null>` replaces the static `credentials[]` array
  - `unresolvedStreak` counter per (userId, venue); after `RECONCILIATION_UNRESOLVED_DOWNGRADE_AFTER` consecutive misses (default `60`), the auto-downgrade hook fires
  - `ReconciliationService` wires `onUnresolvedDowngrade` to write `user_trading_preferences.runtime_lock = "read_only_until=…"` and emit `[Trading] {"stage":"reconciliation_health",...}`
  - New stage `reconciliation_health` + `emitReconciliationHealth` helper
  - Log spam fixed: previously `No credentials found …` every 5 s; now once at streak=1, every 10th attempt thereafter
- **M6 — risk-limits save toast** already worked via the existing `useUpdateTradingPreferences` invalidation; verified no-regression
- **M7 — full-mask API keys** — `getSecretPreview` returns `••••••••` instead of leaking the first 4 chars

### Wave 2 — Features

- **F6 — requestId-anchored CEX continuation bypass**
  - `runtime.ts` precheck gains a third bypass (after anonymous + comprehensive): when a recent CEX-workflow memory in this room flagged `cexAwaitingClarification` or carries a `cexRequestId`, AND the current user message is short (≤120 chars OR a pure number/yes/no), classification skips the LangGraph precheck and routes straight to `handleCEXWorkflowMessage`
  - `cexWorkflowMessageHandler.createFinalResponse` stamps `metadata.cexRequestId` and `metadata.cexAwaitingClarification` on every outbound memory
  - Feature flag: `CEX_DETERMINISTIC_BYPASS=true` (default; flip to `false` to roll back via env)
  - Telemetry: `[Routing] {"stage":"cex_bypass","hit":...}` on every classification turn
- **F7 — canonical-intent validator hardening**
  - `canonicalIntentSchema.superRefine` rejects: `stop_limit` without both prices, `time_in_force=GTD` without `end_time`, `post_only` outside `limit`/`stop_limit`, `margin_type` without `margin_action`
  - LLM extractor + regex-confidence gate **deferred** to a follow-up PR
- **F8 — mode-aware stream copy**
  - New `cexStreamMessages.ts` table with `{en, zh-CN} × {live, paper, shadow}` for `orderSubmit`, `riskCheck`, `approvalRequest` stages
  - Paper-mode order-submit copy never says "exchange"; shadow says "hypothetical"
- **F9 — Binance margin (CROSS + ISOLATED)**
  - `signedMarginOrderPost` (signed POST to `/sapi/v1/margin/order`) in `binanceMargin.ts`
  - `precheckIsolatedAccountOpened` (GET `/sapi/v1/margin/isolated/account`) refuses ISOLATED submits when the user hasn't opened the symbol's isolated account
  - `BinanceOrdersService.createOrder` dispatches to `createMarginOrder` when `params.margin_type` is set; previously threw
  - `isIsolated` wire field, `sideEffectType` derived from `marginActionToSideEffect`
  - Paper / shadow margin orders route through the existing paper venue (F3) and preserve margin metadata on the order record
  - `spec/binance-implemented-endpoints.json` updated
- **F10 — Trade compose button** — minimal scaffold: a Trade button next to Send in `chat.tsx` prefills the chat input with a templated NL trade prompt so the existing CEX workflow opens the order editor with `accountSnapshot` pre-fetched. Full in-place modal deferred.
- **F11 — Coinbase parity audit** — audit conclusion: Coinbase Advanced Trade's `/api/v3/brokerage/orders` accepts the canonical `order_configuration` JSON verbatim, so `stop_limit_stop_limit_gtc/gtd`, TIF suffixes, and `post_only` on `limit_limit_gtc` work natively. F7 validators apply uniformly. Margin remains Binance-only (Coinbase Advanced Trade does not expose spot margin in the canonical shape). Documented in `spec/coinbase-implemented-endpoints.json` under `f11ParityNotes`.
- **M1** — anaphor patterns extended with `my latest`, `my last`, `the recent` (and zh-CN equivalents)
- **M3** — classifier template gains explicit short-circuit rule: trivial single-asset price questions route to `REGULAR_MESSAGE`, not the LangGraph Task Chain
- **M2 / M4 / M5** — deferred to a follow-up polish PR

### Env vars (additions)

- `PAPER_ORDER_TTL_SECONDS` (F3, default `86400`)
- `RECONCILIATION_UNRESOLVED_DOWNGRADE_AFTER` (F5, default `60`)
- `CEX_DETERMINISTIC_BYPASS` (F6, default `true`)
- `CEX_LLM_EXTRACTOR_MODEL` (F7 future, default `claude-haiku-4-5-20251001`)
- `USER_FEATURE_RISK_PHRASE_FILTER` (F2, default `true`)

### Tests

- `packages/plugin-cex/__tests__/`: +`templates.modeBadge.test.ts` (F1), +`paperLedger.test.ts` (F3), +`tradingEvents.invariantFields.test.ts` (F4), +`reconciliation.perUserCreds.test.ts` (F5), +`canonicalIntent.validators.test.ts` (F7), +`binanceMargin.dispatch.test.ts` (F9)
- `packages/core/__tests__/`: +`cexResultFormatting.modeBadge.test.ts` (F1), +`userFeatureManager.adversarial.test.ts` (F2), +`cexStreamMessages.test.ts` (F8)
- All 383 plugin-cex tests pass; all newly added core tests pass.

### Operator follow-ups before launch

See `docs/AUTOTRADING_SAFETY.md` §6. Pay particular attention to:

1. Update CloudWatch metric filters: `stake` field semantics changed from `read_only|write` to `live|paper|shadow`. The old `read_only|write` info now lives on `tool_capability`.
2. Verify TTL indexes on `paper_orders` and `paper_fills` in production DocumentDB.
3. Set `EXCHANGE_TOKEN_ENCRYPTION_KEY` (already required pre-Wave-1).
4. 48 h staging soak between Wave 1 and Wave 2 (kept as a single feature branch here for review velocity; operator decides whether to ship as one merge or split).

---

## Autotrading subsystem uplift — Phase 2-5 + integration uplift (2026-05-17)

### Phase 2 — Reconciliation + per-symbol locks (P0)
- `pending_orders_ledger` MongoDB collection with state-transition guard
- `ReconciliationService` registered at agent startup — Binance + Coinbase user-data WS consumers with auto-reconnect, listenKey refresh, 23h re-auth; REST fallback poller; per-symbol FIFO trading lock (5-min stale TTL via `.unref()` timer)
- Wired `acquireOrderLock` + `trackOrder` into `cexWorkflowMessageHandler.executeAction`
- `ServiceType.TRADING_RECONCILIATION` + `ITradingReconciliationService` interface in core (no circular dep with plugin-cex)

### Phase 3 — ADK trading sub-agent (P1)
- Bounded tool registry: `get_balance`, `get_orders`, `get_fills`, `create_order`, `cancel_order`, `amend_order`, `preview_order`
- Deterministic intent classifier + parameter extractor (EN + zh-CN; CJK-safe regex — no `\b` around CJK chars)
- Tool exposure gating: `visibleTools()` hides write tools when `stake=read_only` OR `kill_switch_active`; venue locked via `AdkRuntimeContext`
- Evaluator suites: macro-F1 ≥ 0.92, write-action precision ≥ 0.95
- **ADK fast-path wired** into `generateLLMResponse` — read-only actions (`get_balance`, `get_orders`, `get_fills`) skip the LLM entirely when the classifier matches confidently; write actions still route through the LLM for the existing approval UX preamble

### Phase 4 — Strategy DSL + Paper venue + Shadow mode (P1)
- `packages/plugin-cex/src/strategy/` — zod-validated DSL (identity, universe, signals, entries, exits, risk, operations, resilience)
- Heuristic NL → DSL compiler (DCA, RSI mean-revert; EN + zh-CN)
- Pure-function `strategyRuntime` emits canonical intents per trigger
- `paperVenue.ts` — full `ExchangeService` implementation with `linear_bps` + `book_walk` slippage models
- `shadowDecisions.ts` — record schema + in-memory writer + divergence ratio aggregation

### Phase 5 — Backtest + Memory routing + KB ranking (P2)
- `packages/plugin-cex/src/backtest/` — OhlcvBar types, deterministic synthetic bar generator, CSV parser, SMA/EMA/RSI/ATR indicators, metrics (Sharpe, Sortino, max-drawdown, win-rate, profit-factor, fee+slippage adjusted, turnover)
- Runner with in-sample / OOS split + per-regime metrics; **look-ahead bias structurally prevented** — only references `bars[0..i]`
- `memory/memoryRouter.ts` — locale-aware preference + recent-trade injection
- **Memory router wired** into the LLM prompt context (`{{memoryContext}}` injection in `cexMessageTemplate`)
- `ranking/hybridRetriever.ts` — BM25 + cosine dense + RRF + MMR + final rerank by trust × freshness × portfolio relevance (A/B/C trust tiers)

### Integration uplift (this round)
- **Structured `tradingEvents` emitter wired** into `cexWorkflowMessageHandler` — `[Trading]` JSON log lines now fire at `preprocess`, `risk_check`, `idempotency`, `lock_acquire`, `lock_release`, `approval_request`, `approval_decision`, `order_submit`, `order_ack`, `order_error`, `clarification_request`, `intent_classified`. CloudWatch metric filters can consume the invariant field set (`request_id`, `intent_hash`, `userId`, `venue`, `symbol`, `side`, `notional_usd`, `locale`, `stake`, `decision`, `rules_fired`, `latency_ms`).
- **Multi-turn cancel-order resolver** — `resolveAnaphoricOrderId` parses recent assistant memories for displayed order IDs; auto-fills `order_ids` when the user says "cancel this order", "the one you just showed", "撤销那个订单", etc. Wired into `parseResponse`.
- **Inline order-table UX**: `MarkdownRenderer` now formats trading-table cells with side coloring (BUY green / SELL red), status badges (NEW, FILLED, CANCELLED, etc.), truncated order IDs with click-to-copy, and an inline **Cancel** chip per open-order row that dispatches `sentiedge:chat-send` for one-click cancellation.
- **`chat.tsx` chat-send event bridge** — accepts external override text via `sentiedge:chat-send` without disturbing the user's currently-typed input.

### `CEXSpecProvider` API additions
- `emitTradingEvent(event)` — structured log line emitter
- `runTradingSubAgent(input)` — ADK sub-agent entry point (canonical intent or clarification)
- `resolveAnaphoricOrderId(input)` — multi-turn order-ID resolver
